...
class Medallium(commands.Cog) :
    ...
    async def medallium(self, ctx = commands.Context, user : discord.User = None ):
        ...
        main_embed.set_author(name="/!\ Étant donné que le bot approche des 100 serveurs, nous risquons de perdre l'accès aux commandes préfixées (ex. `.bkai`). Nous recommandons donc d'utiliser les commandes slash (ex. `/bkai`).")
